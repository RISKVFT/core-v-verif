...
*/
void *portable_malloc(unsigned int size) {
	return malloc(size);
}
...
#else
    // Defined for RISCV
    #define NSECS_PER_SEC 100000000 // TODO: What freq are we assuming? In testbench of core-v-verif the clk_freq is 100MHz (period 10ns)
	#define EE_TIMER_TICKER_RATE 1000 // TODO: What is this?
	#define CORETIMETYPE clock_t // mcycle è 0xC00 ??
    #define read_csr() ({ unsigned int tmp; \
       __asm__ volatile ("csrr %0, mcycle" : "=r"(tmp)); \
	   printf("THIS IS THE START TIME: %d\n", tmp); \
       tmp; })
    #define GETMYTIME(_t) (*_t=read_csr())
	#define MYTIMEDIFF(fin,ini) ((fin)-(ini))
	#define TIMER_RES_DIVIDER 1
	#define SAMPLE_TIME_IMPLEMENTATION 1
#endif
...
void start_time(void) {
    ...
    __asm__ volatile("csrw 0x320, %0" ::"r"(inhibit)); // questa istruzione serve ad abilitare il performance counter che conta i cicli del core
...
secs_ret time_in_secs(CORE_TICKS ticks) {
	secs_ret retval=((secs_ret)(ticks)) / (secs_ret)EE_TICKS_PER_SEC; // value scaled up of 10^6 in order to print correctly in core_main.c
	return retval;
}
// these two functions have been added by Marcello to print correctly the time
// added by Marcello
secs_ret time_in_secs_int_part(CORE_TICKS ticks) { 
	secs_ret int_part = ((secs_ret)(ticks)) / (secs_ret)EE_TICKS_PER_SEC; 
	return int_part;
}
// added by Marcello
secs_ret time_in_secs_decim_part(CORE_TICKS ticks) { // value scaled up of 10^6 in order to print correctly in core_main.c
	secs_ret decim_part = ((((secs_ret)(ticks)) % (secs_ret)EE_TICKS_PER_SEC)) / (secs_ret)(EE_TICKS_PER_SEC/1000000) ;
	return decim_part;
}
#else
